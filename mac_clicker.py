...
# -*- coding: utf-8 -*-
"""
Auto Clicker (Mac 極速自動連點 & 動作錄製高對比暗夜版)
特色：
1. 解決 Mac 系統 Aqua 引擎白底白字問題，全介面純黑化自訂渲染
2. 支援「極速自動連點 (1~1000 CPS)」與「動作錄製 (Macro Recorder)」雙分頁
3. 動作錄製支援：
   - 儲存動作 (輸入自訂名稱保存多組巨集) 與 刪除動作
   - 繫結快速鍵 (預設 F9 錄製開關、F8 執行開關，支援自訂按鍵綁定)
   - 專業穩定錄製：精確記錄鍵盤按鍵與滑鼠點擊瞬間 (X, Y)，排除肥大雜亂的滑鼠連續位移軌跡
   - 支援重複次數設定 (0 為無限循環) 與倍速播放
4. 連點與錄製皆具備防誤觸機制 (放開按鍵確認 + 0.5 秒冷卻時間)
"""
...
try:
    ...
    print("尚未安裝 pynput，請先執行 pip3 install pynput")
    # 不強制退出以利於無依賴環境測試驗證
    mouse = None
    keyboard = None
    Button = None
...
C_TEXT_WHITE = "#ffffff"  # 純白文字 (超高對比)
C_TEXT_CYAN  = "#00e5ff"  # 霓虹青藍 (強調、數值)
...
class DarkButton(tk.Label):
    """自訂暗色按鈕：徹底解決 Mac 系統原生按鈕會強制白底的問題"""
    def __init__(self, parent, text, command=None, bg=C_BTN_NORMAL, fg=C_TEXT_WHITE, 
                 hover_bg=C_BTN_HOVER, active_bg=C_BTN_ACTIVE, font=("Helvetica Neue", 10, "bold"), 
...
class AutoClickerApp:
    ...
    def _build_ui(self):
        ...
        self.tab_btn_clicker = DarkButton(
            ...
            font=("Helvetica Neue", 10, "bold"), padx=14, pady=6
        )
        ...
        self.status_label = tk.Label(
            ...
            font=("Helvetica Neue", 12, "bold"), fg=C_RED, bg=C_BG_CARD
        )
    ...
    def _build_clicker_tab(self, parent):
        ...
        tk.Label(
            ...
            font=("Helvetica Neue", 10, "bold"), fg=C_TEXT_CYAN, bg=C_BG_CARD
        ...
        DarkButton(
            preset_bar, text="➕ 儲存當前 CPS 為檔位", font=("Helvetica Neue", 9, "bold"),
            bg="#163828", fg=C_GREEN, hover_bg="#1d4d37", padx=8, pady=4,
            command=self._add_current_preset
        ).pack(side=tk.LEFT)

        DarkButton(
            preset_bar, text="🗑️ 刪除檔位", font=("Helvetica Neue", 9),
        ...
        tk.Label(
            ...
            font=("Helvetica Neue", 10, "bold"), fg=C_TEXT_WHITE, bg=C_BG_CARD
        ).pack(side=tk.LEFT)

        self.cps_entry = tk.Entry(
            cps_head, textvariable=self.cps_var, width=7, font=("Helvetica Neue", 12, "bold"),
        ...
        self.uncapped_toggle_btn = DarkButton(
            toggle_frame, text="⚡ 極速狂暴模式：關閉", font=("Helvetica Neue", 9, "bold"),
        ...
        self.jitter_toggle_btn = DarkButton(
            toggle_frame, text="隨機延遲 (Jitter)：關閉", font=("Helvetica Neue", 9),
        ...
        tk.Label(mouse_row, text="連點按鍵:", font=("Helvetica Neue", 10), fg=C_TEXT_WHITE, bg=C_BG_CARD).pack(side=tk.LEFT)
        ...
        tk.Label(hotkey_row, text="連點開關熱鍵:", font=("Helvetica Neue", 10), fg=C_TEXT_WHITE, bg=C_BG_CARD).pack(side=tk.LEFT)

        self.hotkey_btn_click = DarkButton(
            hotkey_row, text=f"[{self.hotkey_click_display}] 點我更改熱鍵", font=("Helvetica Neue", 10, "bold"),
        ...
        self.main_btn_click = DarkButton(
            parent, text="▶ 啟動連點 (或按熱鍵)", font=("Helvetica Neue", 12, "bold"),
    ...
    def _build_recorder_tab(self, parent):
        ...
        tk.Label(
            ...
            font=("Helvetica Neue", 10, "bold"), fg=C_TEXT_PURPLE, bg=C_BG_CARD
        ).pack(anchor=tk.W, pady=(0, 8))

        row1 = tk.Frame(hk_card, bg=C_BG_CARD)
        row1.pack(fill=tk.X, pady=(0, 6))
        tk.Label(row1, text="錄製開關熱鍵:", font=("Helvetica Neue", 10), fg=C_TEXT_WHITE, bg=C_BG_CARD).pack(side=tk.LEFT)
        self.hotkey_btn_record = DarkButton(
            row1, text=f"[{self.hotkey_record_display}] 點我更改熱鍵", font=("Helvetica Neue", 9, "bold"),
        ...
        tk.Label(row2, text="回放執行熱鍵:", font=("Helvetica Neue", 10), fg=C_TEXT_WHITE, bg=C_BG_CARD).pack(side=tk.LEFT)
        self.hotkey_btn_play = DarkButton(
            row2, text=f"[{self.hotkey_play_display}] 點我更改熱鍵", font=("Helvetica Neue", 9, "bold"),
        ...
        tk.Label(row3, text="錄製範圍:", font=("Helvetica Neue", 10), fg=C_TEXT_WHITE, bg=C_BG_CARD).pack(side=tk.LEFT)
        
        self.btn_toggle_mouse_record = DarkButton(
            row3, text="✓ 鍵盤 + 滑鼠點擊瞬間", font=("Helvetica Neue", 9, "bold"),
        ...
        self.current_rec_label = tk.Label(
            mgmt_head, text="目前未錄製動作", font=("Helvetica Neue", 9), fg=C_TEXT_DIM, bg=C_BG_CARD
        )
        self.current_rec_label.pack(side=tk.RIGHT)

        # 動作列表 Listbox
        self.macro_listbox = tk.Listbox(
            mgmt_card, height=4, font=("Helvetica Neue", 10),
        ...
        tk.Label(opt_row, text="重複次數 (0=無限循環):", font=("Helvetica Neue", 9), fg=C_TEXT_DIM, bg=C_BG_CARD).pack(side=tk.LEFT)
        self.loops_entry = tk.Entry(
            opt_row, textvariable=self.macro_loops_var, width=5, font=("Helvetica Neue", 10, "bold"),
            bg=C_BG_INPUT, fg=C_TEXT_CYAN, insertbackground=C_TEXT_CYAN,
            relief="solid", bd=1, highlightbackground=C_BORDER, justify=tk.CENTER
        )
        self.loops_entry.pack(side=tk.LEFT, padx=(6, 16))

        tk.Label(opt_row, text="回放速度:", font=("Helvetica Neue", 9), fg=C_TEXT_DIM, bg=C_BG_CARD).pack(side=tk.LEFT)
        for spd in [1.0, 1.5, 2.0]:
            btn = DarkButton(
                opt_row, text=f"{spd}x", font=("Helvetica Neue", 9),
        ...
        self.btn_main_record = DarkButton(
            ctrl_frame, text=f"⏺ 開始錄製 (按 [{self.hotkey_record_display}])", font=("Helvetica Neue", 11, "bold"),
        ...
        self.btn_main_play = DarkButton(
            ctrl_frame, text=f"▶ 執行動作 (按 [{self.hotkey_play_display}])", font=("Helvetica Neue", 11, "bold"),
            bg="#1d3b66", fg=C_TEXT_WHITE, hover_bg="#274f8a", pady=10,
    ...
    def _render_presets(self):
        ...
        for val in self.presets:
            btn = DarkButton(
                self.preset_box, text=f"{val} CPS", font=("Helvetica Neue", 9, "bold"),
